...
# Generate 8 Weeks of Programming for All Tracks (Remote Supabase)
# Starting June 9, 2025 (Monday)

echo "🚀 GENERATING 8 WEEKS OF PROGRAMMING (REMOTE)"
echo "=============================================="
echo "📅 Start Date: June 9, 2025 (Monday)"
echo "📊 Duration: 8 weeks"
echo "🎯 Tracks: All 8 tracks"
echo "🌐 Target: Remote Supabase Database"
echo ""

echo "📝 Executing SQL against remote Supabase database..."

# Clear existing sessions for clean regeneration
echo "🧹 Clearing existing sessions..."
supabase db query "DELETE FROM block_exercises;"
supabase db query "DELETE FROM blocks;"
supabase db query "DELETE FROM sessions;"

echo "✅ Existing sessions cleared"

# Create base exercises if they don't exist
echo "💪 Creating base exercises..."
supabase db query "
...
# Generate programming for each track
echo ""
...
BEGIN
    ...
    FOR week_num IN 1..8 LOOP
        ...
        -- Tuesday through Saturday sessions
        INSERT INTO sessions (track_id, day_of_week, week_number, session_type, name, description, duration_minutes, intensity_level, is_published)
        VALUES 
        (track_id, 2, week_num, 'Olympic', 'Tuesday Olympic', 'Clean & Jerk Focus', 60, 7, true),
        (track_id, 3, week_num, 'Gymnastics', 'Wednesday Gymnastics', 'Pull-ups + Conditioning', 55, 8, true),
        (track_id, 4, week_num, 'Strength', 'Thursday Strength', 'Deadlift + Accessory', 60, 7, true),
        (track_id, 5, week_num, 'Conditioning', 'Friday Metcon', 'High Intensity Mixed Modal', 45, 9, true),
        (track_id, 6, week_num, 'Conditioning', 'Saturday Grind', 'Longer Time Domain', 50, 6, true);
...
BEGIN
    ...
    FOR week_num IN 1..8 LOOP
        -- Monday AM & PM sessions
        INSERT INTO sessions (track_id, day_of_week, week_number, session_type, name, description, duration_minutes, intensity_level, is_published)
        VALUES 
        (track_id, 1, week_num, 'Strength', 'Monday AM Strength', 'Heavy Back Squat', 45, 9, true),
        (track_id, 1, week_num, 'Conditioning', 'Monday PM Metcon', 'Sprint Intervals', 30, 10, true);
        
        -- Tuesday through Sunday
        INSERT INTO sessions (track_id, day_of_week, week_number, session_type, name, description, duration_minutes, intensity_level, is_published)
        VALUES 
        (track_id, 2, week_num, 'Olympic', 'Tuesday Olympic', 'Snatch Complex', 60, 8, true),
        (track_id, 3, week_num, 'Gymnastics', 'Wednesday Gymnastics', 'Muscle-up Development', 50, 7, true),
        (track_id, 4, week_num, 'Conditioning', 'Thursday Metcon', 'Competition Simulation', 40, 10, true),
        (track_id, 5, week_num, 'Strength', 'Friday Strength', 'Clean & Jerk Max', 50, 9, true),
        (track_id, 6, week_num, 'Conditioning', 'Saturday Grind', 'Endurance Test', 60, 8, true),
        (track_id, 7, week_num, 'Recovery', 'Sunday Recovery', 'Active Recovery + Skills', 40, 3, true);
...
BEGIN
    ...
    FOR week_num IN 1..8 LOOP
        INSERT INTO sessions (track_id, day_of_week, week_number, session_type, name, description, duration_minutes, intensity_level, is_published)
        VALUES 
        (track_id, 1, week_num, 'Strength', 'Max Effort Upper', 'Heavy Press Variation', 60, 9, true),
        (track_id, 2, week_num, 'Strength', 'Max Effort Lower', 'Heavy Squat/Deadlift', 60, 9, true),
        (track_id, 4, week_num, 'Power', 'Dynamic Effort Upper', 'Speed Press Work', 50, 7, true),
        (track_id, 5, week_num, 'Power', 'Dynamic Effort Lower', 'Speed Squat Work', 50, 7, true);
    END LOOP;
END \$\$;
"
...
# Get final count using Supabase CLI
echo ""
echo "📊 FINAL SUMMARY"
echo "=================="

# Query session counts per track
supabase db query "
SELECT 
    wt.name,
...
total_sessions=$(supabase db query "SELECT COUNT(*) FROM sessions;" --output csv | tail -n 1)
